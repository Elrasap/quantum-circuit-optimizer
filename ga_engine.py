...
def run_quantum_optimization(run_id=None):
    ...
    # Access Qasm File
    qc = QuantumCircuit.from_qasm_file("C:/Users/Pasarle/Desktop/Abasin/Experiment/MQTBench_2024-09-05-12-48-08/qft_indep_qiskit_6.qasm")
    qc = qc.decompose()
    ...
    # Funktion zur Erzeugung eines Graphen mit gerichteten, gewichteten Kanten
    def create_graph_from_circuit(circuit):
        dag = circuit_to_dag(circuit)
        graph = nx.Graph()

        # Füge Knoten für jedes Qubit hinzu
        for qubit in circuit.qubits:
            graph.add_node(qubit.index)

        # CNOT-Gates zu gewichteten Kanten hinzufügen
        for gate in dag.two_qubit_ops():
            control, target = [q.index for q in gate.qargs]
            # Setze eine höhere Gewichtung für Control zu Target
            graph.add_edge(control, target, weight=2)  # Priorität für gleiche Partition von Control zu Target
            graph.add_edge(target, control, weight=1)  # Niedrigere Priorität für Target zu Control

        return graph

    # Funktion zur Partitionierung des gewichteten Graphen mit METIS
    ...
    # Zähle globale Gates (Schnittkanten) nach der Partitionierung
    def count_global_gates_with_kl_algorithm(circuit, partition):
        global_count = 0
        for gate in circuit.data:
            # Save Statevector wird mitgezählt
    ...
    # Erstelle gewichteten Graphen für den Schaltkreis
    ...
    def evaluate_fidelity(circuit, qc, backend):
        ...
        # Fidelity between initial Circuit and optimized Circuit
    ...
    def fitness_calculation(circuit, partition, qc, backend, weight_gates=0.5, weight_fidelity=0.5, fidelity_threshold=0.99):
        ...
        return (weight_gates * global_count) + (weight_fidelity * (1 - fidelity) + 1e-5)

    # Roulette Wheel Selection
    def roulette_wheel_selection(population, num_circuits, qc, backend):
        # Fitness for each Circuit
        ...
        # Probability for Circuit (good Fitness is small)
        ...
        # Put Circuit in P1 and P2 alternately
        p1 = []
        p2 = []
    ...
    def mutation(circuit, r_mut):
        ...
        # Remove Gate in Circuit Mutation
    ...
    # Save fintess value per generation
    ...
    for generation in range(generations):
        ...
        # Compare Best Circuit with current circuit
        # 
        if best_score < current_best_fitness: #best_fidelity >= 0.99 and
            # New best circuit
            current_best_circuit = best_circuit
            current_best_fitness = best_score
            current_best_fidelity = best_fidelity

            # Save all fitness values for this generation
    ...
    # Plot individual fitness values
    for fitness_value in fitness_values_per_generation[0]:
        plt.plot(generations_list, [fitness_value] * len(generations_list), alpha=0.6, label='_nolegend_')

    # Plot mean fitness over generations
    mean_line = plt.plot(generations_list, mean_fitness_per_generation, color='blue', label='Mean Fitness', linewidth=2)

    plt.fill_between(generations_list, np.min(fitness_values_per_generation, axis=1), np.max(fitness_values_per_generation, axis=1), color='lightblue', alpha=0.5)
    plt.title(f'Fitness per generation - {run_id}')
    ...
    plt.title(f'Best Fidelity per Generation - {run_id}')
    plt.xlabel('Generation')
    plt.ylabel('Fidelity')
    plt.legend()
    plt.grid(True)
...
def run_quantum_optimization(run_id=None):
    ...
    # Access Qasm File
    #qc = QuantumCircuit.from_qasm_file("C:/Users/Pasarle/Desktop/Abasin/Experiment/MQTBench_2024-09-05-12-48-08/qft_indep_qiskit_4.qasm")
    #qc = qc.decompose()
    ...
    # Plot individual fitness values
    for fitness_value in fitness_values_per_generation[0]:
        plt.plot(generations_list, [fitness_value] * len(generations_list), color='lightgray', alpha=0.6, label='_nolegend_')
